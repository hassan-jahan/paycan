...
class ServiceOrderFulfilledNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function via(object $notifiable): array
    {
        ...
        if (! $templateService->isEnabled('service_order_fulfilled')) {
            return [];
        }
    ...
    {
        $templateService = app(NotificationTemplateService::class);
        $template = $templateService->getTemplate('service_order_fulfilled');
        ...
        $variables = [
            ...
            'service_code' => $this->fulfillment->meta['service_code'] ?? 'N/A',
            'instructions' => $this->fulfillment->meta['instructions'] ?? 'N/A',
            'valid_until' => $this->fulfillment->meta['valid_until'] ?? 'N/A',
        ];
