...
export class SubscriptionsModal {
  private sdk: PayCan;
  private options: SubscriptionsModalOptions;
   ...
   */
  private async loadSubscriptions(page: number = 1): Promise<void> {
    this.loading = true;
    try {
   ...
   */
  private async handleChangePlan(subscriptionId: string, priceId?: string): Promise<void> {
   ...
   */
  private getSubscriptionsList(): string {
   ...
   */
  private getSubscriptionCard(sub: Subscription): string {
   ...
   */
  private getSubscriptionActions(sub: Subscription): string {
    ...
    actionButtons.forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          switch (action) {
            case 'cancel':
              if (id) await this.cancelSubscription(id);
              break;
            case 'resume':
              if (id) await this.resumeSubscription(id);
              break;
            case 'change-plan':
              if (id) await this.handleChangePlan(id, target.dataset.priceId);
              break;
            case 'change-payment':
